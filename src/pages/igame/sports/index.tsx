...
const SportsGamePage: NextPage<SportsPageProps> = ({ games, hotWeekGames }) => {
      ...
      <div className="flex bg-neutral-0 container pt-10 gap-6 flex-col xl:flex-row">
        <section className="w-full xl:w-2/3 hover:cursor-pointer h-full" onClick={() => onClickGame(hotWeekGames[0]?.id)}>
          ...
          <div>
            ...
            <p className="text-[#666666]">Hot Tuần • Thể Thao • {hotWeekGames[0]?.numberOfPlayer} người tham gia</p>
          </div>
        </section>
        <section className="hidden sm:flex xl:flex-col w-full xl:w-1/3 gap-4">
          <div className="w-1/2 xl:w-full hover:cursor-pointer" onClick={() => onClickGame(hotWeekGames[1]?.id)}>
            ...
            <div>
              ...
              <p className="text-[#666666]">Hot Tuần • Thể Thao • {hotWeekGames[1]?.numberOfPlayer} người tham gia</p>
            </div>
          </div>
          <div className="w-1/2 xl:w-full hover:cursor-pointer" onClick={() => onClickGame(hotWeekGames[2]?.id)}>
            ...
            <div>
              ...
              <p className="text-[#666666]">Hot Tuần • Thể Thao • {hotWeekGames[2]?.numberOfPlayer} người tham gia</p>
            </div>
          </div>
        </section>
      </div>
      <div className="bg-neutral-0">
        <SectionIgame classNameTitle={'hidden sm:flex'} title={'GAME THỂ THAO'} className="game-hot container py-10 xl:py-20">
...
SportsGamePage.getLayout = function (page) {
  return (
    <>
      <LayoutIgame pageMobileTitle={'Game thể thao'}>{page}</LayoutIgame>
      <ChatBoxLazy />
    </>
  );
};

const getStaticProps = getServerPropsWithTranslation<SportsPageProps>(async () => {
  return {
    props: {
