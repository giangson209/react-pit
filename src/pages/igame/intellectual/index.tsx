...
type IntellectualPageProps = {
  games: IGame[];
  hotWeekGames: IGame[];
};

const IntellectualGamePage: NextPage<IntellectualPageProps> = ({ games, hotWeekGames }) => {
      ...
      <div className="flex bg-neutral-0 container pt-10 gap-6 flex-col xl:flex-row">
        <section className="w-full xl:w-2/3 hover:cursor-pointer h-full" onClick={() => onClickGame(hotWeekGames[0]?.id)}>
          ...
          <div>
            ...
            <p className="text-[#666666]">Hot Tuần • Trí Tuệ • {hotWeekGames[0]?.numberOfPlayer} người tham gia</p>
          </div>
        </section>
        <section className="xl:flex-col w-full xl:w-1/3 gap-4 hidden sm:flex">
          <div className="w-1/2 xl:w-full hover:cursor-pointer" onClick={() => onClickGame(hotWeekGames[1]?.id)}>
            ...
            <div>
              ...
              <p className="text-[#666666]">Hot Tuần • Trí Tuệ • {hotWeekGames[1]?.numberOfPlayer} người tham gia</p>
            </div>
          </div>
          <div className="w-1/2 xl:w-full hover:cursor-pointer" onClick={() => onClickGame(hotWeekGames[2]?.id)}>
            ...
            <div>
              ...
              <p className="text-[#666666]">Hot Tuần • Trí Tuệ • {hotWeekGames[2]?.numberOfPlayer} người tham gia</p>
            </div>
          </div>
        </section>
      </div>
      <div className="bg-neutral-0">
        <SectionIgame classNameTitle={'hidden sm:flex'} title={'GAME TRÍ TUỆ'} className="game-hot container py-10 xl:py-20">
...
IntellectualGamePage.getLayout = function (page) {
  return (
    <>
      <LayoutIgame pageMobileTitle={'Game trí tuệ'}>{page}</LayoutIgame>
      <ChatBoxLazy />
    </>
  );
};

const getStaticProps = getServerPropsWithTranslation<IntellectualPageProps>(async () => {
  return {
    props: {
...
export default IntellectualGamePage;
